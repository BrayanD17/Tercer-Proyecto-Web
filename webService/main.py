...
#Servicio para obtener los datos y cargarlos en la vista general del dashboard
@app.get("/dashboard_overview/", response_model=DashboardData)
async def get_dashboard_data(db:Session=Depends(get_db), current_user: User =Depends(get_current_user)):
    try:
        #Usuario logeado
        user_id=current_user.id

        #Consultar los datos más recientes de cada tabla usando MAX para las fechas
        weights= db.query(Weight).filter(Weight.userId==user_id).order_by(Weight.date.desc()).first()
        heights=db.query(Height).filter(Height.userId==user_id).order_by(Height.date.desc()).first()
        water=db.query(WaterConsumption).filter(WaterConsumption.userId==user_id).order_by(WaterConsumption.date.desc()).first()
        exercise=db.query(Exercise).filter(Exercise.userId==user_id).order_by(Exercise.date.desc()).first()
        steps=db.query(DailySteps).filter(DailySteps.userId==user_id).order_by(DailySteps.date.desc()).first()
        fat=db.query(BodyFatPercentage).filter(BodyFatPercentage.userId==user_id).order_by(BodyFatPercentage.date.desc()).first()
        body_compositions=db.query(BodyComposition).filter(BodyComposition.userId==user_id).order_by(BodyComposition.date.desc()).first()

        #Respuesta
        response_data=DashboardData(
            weights=WeightData.from_orm(weights) if weights else None,
            heights=HeightData.from_orm(heights) if heights else None,
            water_consumptions= WaterConsumptionData.from_orm(water) if water else None,
            exercises=ExerciseData.from_orm(exercise) if exercise else None,
            daily_steps=DailyStepsData.from_orm(steps) if steps else None,
            body_fat_percentages=BodyFatPercentageData.from_orm(fat) if fat else None,
            body_compositions=BodyCompositionData.from_orm(body_compositions) if body_compositions else None
        ...
        raise HTTPException(status_code=500, detail=f"Error al obtener los datos para la vista general: {str(e)}")